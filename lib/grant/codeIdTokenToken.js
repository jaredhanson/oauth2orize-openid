  ...
  function response(txn, res, next) {
    ...
    function doIssueIDToken(tok) {
      ...
      try {
        ...
        if (arity == 5) {
          issueIDToken(txn.client, txn.user, txn.req ? txn.req.scope : undefined, txn.req, issued);
        } else if (arity == 4) {
          issueIDToken(txn.client, txn.user, txn.req ? txn.req.scope : undefined, issued);
        } else { // arity == 3
